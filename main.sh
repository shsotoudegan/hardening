...
apt install -y wget git vim nano bash-completion curl htop iftop jq ncdu unzip net-tools dnsutils \
               ...
               tmux apt-transport-https ca-certificates
sudo snap install nvim --classic
git clone --depth 1 https://github.com/AstroNvim/AstroNvim /home/$USER/.config/nvim
...
# change DNS ------------------------------------------------------------------------
cp /etc/netplan/* $BAC_DIR
cat <<EOT > /etc/netplan/00-installer-config.yaml
# This is the network config written by hardening code
network:
  ethernets:
    enp0s3:
      dhcp4: true
